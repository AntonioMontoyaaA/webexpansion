...
public class CreaExcelMemoriasAsignadas {
	
	public HSSFWorkbook createWorkbook(List<MemoriaAsignadaVO> listaMemorias) throws Exception {

        HSSFWorkbook wb = new HSSFWorkbook();
        HSSFSheet sheet = wb.createSheet("Memorias asignadas");
        ...
        cell.setCellStyle(headerCellStyle);
        cell.setCellValue(new HSSFRichTextString("FECHA VENCIMIENTO"));
        ...
        for(int i = 0; i < listaMemorias.size(); i++) {
        	...
        	if(listaMemorias.get(i).isMdVencida()) {
        		datosCellStyle = datosCellStyleVencidas;
        	} else {
            ...
            cellDatos.setCellStyle(datosCellStyle);
            cellDatos.setCellValue(new HSSFRichTextString(listaMemorias.get(i).getFechaVencimiento()));
        }
