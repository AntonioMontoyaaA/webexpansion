...
public class DespausaReactivaMdAction extends ExpansionAction implements SessionAware, ParameterNameAware {
	...
	public String execute() throws Exception {
		...
		try {
			if(usuario != null) {
				...
				String tipoReactivacion = ServletActionContext.getRequest().getParameter("tipoReactivacion");
				
				final OkHttpClient client = new OkHttpClient();
				FormBody.Builder formBuilder = new FormBody.Builder()
				 .add("mdId", mdId)
				 .add("usuarioId", String.valueOf(usuario.getPerfil().getNumeroEmpleado()))
				 .add("tipoReactivacion", tipoReactivacion);
				
				
				 RequestBody formBody = formBuilder.build();
				 Request request = new Request.Builder()
						 .url(sp.getPropiedad("accionDespausaEstatus"))
		                 .post(formBody)
		                 .build();
